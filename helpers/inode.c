...
int free_inode(int inum) {
    // Get the inode and mark it as free
    struct inode* inode = get_inode(inum);
    inode->used = 0;

    // Update the inode bitmap
    set_bitmap(inode_bitmap, inum, 0);

    return 0; // Success
}

int free_data_blocks(uint32_t* data_ptrs, uint32_t* ind_ptrs) {
    // Free direct data blocks
    for (int i = 0; i < NUM_DIRECT_PTRS; i++) {
        if (data_ptrs[i] != 0) {
            set_bitmap(data_bitmap, data_ptrs[i], 0);
        }
    }

    // Free indirect data blocks
    if (ind_ptrs[0] != 0) {
        uint32_t* ind_data_ptrs = (uint32_t*)get_data_block(ind_ptrs[0]);
        for (int i = 0; i < BLOCK_SIZE / sizeof(uint32_t); i++) {
            if (ind_data_ptrs[i] != 0) {
                set_bitmap(data_bitmap, ind_data_ptrs[i], 0);
            }
        }
        set_bitmap(data_bitmap, ind_ptrs[0], 0);
    }

    // Free doubly indirect data blocks
    if (ind_ptrs[1] != 0) {
        uint32_t* ind_ptrs2 = (uint32_t*)get_data_block(ind_ptrs[1]);
        for (int i = 0; i < BLOCK_SIZE / sizeof(uint32_t); i++) {
            if (ind_ptrs2[i] != 0) {
                uint32_t* ind_data_ptrs = (uint32_t*)get_data_block(ind_ptrs2[i]);
                for (int j = 0; j < BLOCK_SIZE / sizeof(uint32_t); j++) {
                    if (ind_data_ptrs[j] != 0) {
                        set_bitmap(data_bitmap, ind_data_ptrs[j], 0);
                    }
                }
                set_bitmap(data_bitmap, ind_ptrs2[i], 0);
